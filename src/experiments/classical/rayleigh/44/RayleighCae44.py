...
def test_bpsk_rayleigh_eq(snr_dBs, Ntest, k, n, device):
    ...
    for ebdb in snr_dBs:
        ...
        # Rayleigh block fading (complex, same logic as AE channel)
        hr = torch.randn(Ntest, device=device) / math.sqrt(2)
        hi = torch.randn(Ntest, device=device) / math.sqrt(2)

        # BPSK placed on real axis, zero imaginary part
        xr = tx
        xi = torch.zeros_like(tx)

        # channel
        yr = hr[:, None] * xr - hi[:, None] * xi + sigma * torch.randn_like(xr)
        yi = hr[:, None] * xi + hi[:, None] * xr + sigma * torch.randn_like(xi)

        # perfect equalization
        h2 = hr ** 2 + hi ** 2 + 1e-12
        xr_eq = (yr * hr[:, None] + yi * hi[:, None]) / h2[:, None]
...
np.savez(
    f"CAE_{n}{k}_vs_BPSK_Rayleigh(new).npz",
