...
class HAEEncoder(nn.Module):
    def __init__(self, M, n, L=3):
        ...
        # Shared-by-parity parameters:
        # one angle for even qubits and one for odd qubits per layer
        self.theta_re = nn.Parameter(torch.randn(L, 2) * 0.1)
        self.theta_im = nn.Parameter(torch.randn(L, 2) * 0.1)
        ...
        def qnode_re(ampvec, weights):
            ...
            for l in range(L):
                even_angle = math.pi * weights[l, 0]
                odd_angle = math.pi * weights[l, 1]
                for i in range(n):
                    angle = even_angle if i % 2 == 0 else odd_angle
...
np.savez("smallpqc74.npz", **results)
print("Saved: smallpqc74.npz")
